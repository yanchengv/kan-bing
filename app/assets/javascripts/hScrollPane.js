(function($){
	...
	$.fn.hScrollPane=function(settings){
		...
		this.each(function(){
			var container=$(this),
				...
				w=843,
				c=settings.moverW || mover.height(),
			...
			handle.bind("mousedown",function(e){
				var x=e.pageY;
				var hx=parseInt(handle.css("top"));
				if(settings.handleCssAlter){$(this).addClass(settings.handleCssAlter);}
				
				$(document).bind("mousemove",function(e){
					var left=e.pageY-x+hx<0?0:(e.pageY-x+hx>=maxlen?maxlen:e.pageY-x+hx);
					handle.stop().css({top:left});
					if(settings.easing){
						mover.stop().animate({
							top:-left/maxlen*(c-w)			
						},{duration:1500,easing:'easeOutQuint',queue:false});
					}else{
						mover.css({top:-left/maxlen*(c-w)});
					}
	...
	$.fn.hScrollPane.defaults = {
		...
		dragable:true,
		easing:true,
		mousewheel:{bind:true,moveLength:105}
	};
	
	$.fn.hScrollPane.move=function(settings,mover,handle,w,c,maxlen,x,nx){
		if(arguments.length==7){
			var left=parseInt(mover.css("top"))+x*settings.mousewheel.moveLength;
		}else{
			var left=parseInt(mover.css("top"))+((nx-x)/w)*(c-w);
		}
		left=left.toFixed(0);
