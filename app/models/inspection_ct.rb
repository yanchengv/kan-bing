...
#根据patient_id查找CT或者核磁ID 以及instance的图片
  def find_instance_by_id patient_id, inspection_type
    ...
    @studys=[]
       if inspection_type=='CT'
         @inspection_cts = InspectionCt.where("patient_id=? and child_type=?",patient_id,'CT').order("checked_at DESC")
        elsif inspection_type=='MR'
          @inspection_cts = InspectionCt.where("patient_id=? and (child_type=? or child_type=?)",patient_id,'MR','MRI').order("checked_at DESC")
        else
          @inspection_cts=[]
        end
  ...
  def find_series_by_studyuid inspection_id, inspection_type
    @series_instance_data=[]

    if inspection_type=='CT'
      @inspection_cts = InspectionCt.where("id=? and child_type=?",inspection_id,'CT').first
    elsif inspection_type=='MR'
      @inspection_cts = InspectionCt.where("id=? and (child_type=? or child_type=?)",inspection_id,'MR','MRI').first
    else
      @inspection_cts=nil
    end
